...
<html>
	<head>
		...
		<link rel="stylesheet" type="text/css" href="css/fs-form.php" />
	...
	<body>
		<div id="fs-form-wrap">
			...
			<form id="fs-form" method="GET" action="clearlamp">
				<label for="fs-LR2ID">LR2ID</label>
				<input id="fs-LR2ID" name="lr2ID" type="text" pattern="[0-9]{0,6}">
				
				<label for="fs-url">Table</label>
				<select id="fs-urlselect" onchange="this.nextElementSibling.value=this.value">
				...
				<input id="fs-url" name="table_url" type="text">
				
				<button id="fs-button">OK</button>
			</form>
		</div>
		...
		<script>
			window.onload = function () {
				...
				<?php
					if($lr2ID !== "" && $table_url !== "") {
						echo '$("#fs-form-wrap").hide();';
						echo '$("#lamp_header").show();';
						echo '$(".wrapper").show();';
					}
				...
				<?php
				if(!empty($datafullstring)) {
					echo (
						'var chart = new CanvasJS.Chart("chartContainer", '.$datafullstring.');
						chart.render();
						$("#sidebar").show();'
					);
				}
